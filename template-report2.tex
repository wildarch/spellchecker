...
\[
P_{KN}(w_i|w_{i-1}) = \frac{max(c(w_{i-1}, w_i) - \delta, 0)}{\sum_w' c(w_i-1, w')} + \lambda_{w_i-1} \cdot P_{KN}(w_i)
\]
...
To calibrate our parameters, we tested the program on a lot of different sentences and checked for incorrect words. We would then make the program print out elaborate information about how the 'incorrect' word was selected (so its type probability, word occurence, bigram with left neighbour, bigram with right neighbour, and combined probability). We used this information to see which parts of the probability calculation were weighed either too much or too little. We then tried out various different parameters for both NO\_ERROR and LAMBDA, in order to get probability variations which made more sense to us, and lead to a better correction rate. After lots of testing and iterating we ended up with the value of 0.8 for NO\_ERROR, and 0.3 for LAMBDA.
